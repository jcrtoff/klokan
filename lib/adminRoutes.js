...
const { anthropic, EXTRACTION_MODEL } = require('./claude');
...
function setupAdminRoutes(app) {
  ...
  app.get('/api/site-content/consent', async (req, res) => {
    try {
      ...
      // Fall back to FR if EN values are empty (unless noFallback requested)
      if (lang === 'en' && !req.query.noFallback && Object.values(result).some(v => !v)) {
  ...
  app.get('/api/site-content/privacy', async (req, res) => {
    try {
      ...
      // Fall back to FR if EN is empty (unless noFallback requested)
      if ((!row || !row.value) && lang === 'en' && !req.query.noFallback) {
  ...
  app.post('/api/admin/translate', requireRole('admin'), async (req, res) => {
    try {
      const { text, targetLang } = req.body;
      if (!text || !text.trim()) return res.status(400).json({ error: 'empty_text' });
      if (!['fr', 'en'].includes(targetLang)) return res.status(400).json({ error: 'invalid_lang' });

      const sourceLang = targetLang === 'en' ? 'French' : 'English';
      const targetLangName = targetLang === 'en' ? 'English' : 'French';
      ...
      const response = await anthropic.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `Translate the following ${sourceLang} text to ${targetLangName}. Return ONLY the translated text, preserving all HTML tags and formatting exactly. Do not add any explanation.\n\n${text}`
        }]
      });
      ...
      console.error('POST /api/admin/translate error:', err.message);
      res.status(500).json({ error: 'internal_error' });
    }
  });
