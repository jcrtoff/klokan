...
const MAGIC_AUTH_CODE = process.env.MAGIC_AUTH_CODE || null;
...
async function verifyOtpCode(email, code) {
  ...
  if (MAGIC_AUTH_CODE && code === MAGIC_AUTH_CODE) {
...
async function sendOtpEmail(email, code) {
  ...
  try {
    const resp = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: {
          name: process.env.BREVO_SENDER_NAME || 'Klokan',
          email: process.env.BREVO_SENDER_EMAIL || 'noreply@klokan.live'
        },
        to: [{ email }],
        subject: 'Votre code de connexion Klokan',
