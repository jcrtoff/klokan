...
function getSystemPrompt(brokerName) {
  ...
  return `Tu es l'assistante virtuelle IA de ${name}, courtier immobilier agréé à Montréal (OACIQ). Tu fais partie de Klokan, la plateforme intelligente de ${name}.
...
async function extractLeadProfile(conversation, brokerName) {
  const conversationText = conversation
    .map(e => {
      ...
      if (e.role === 'assistant') return `AI: ${e.content}`;
