...
async function getFilteredSessionList(brokerId, role) {
  ...
  if (role === 'admin') {
    ...
    where = { assignedBrokerId: { in: allIds } };
  } else {
    where = { OR: [{ assignedBrokerId: null }, { assignedBrokerId: brokerId }] };
  }
...
async function enrichSessionList(summaries) {
  ...
  for (const s of summaries) {
    ...
    if (cached) {
      ...
      for (let i = cached.conversation.length - 1; i >= 0; i--) {
        if (cached.conversation[i].role === 'user') {
          s.lastMessage = cached.conversation[i].content;
          break;
        }
      }
    } else {
