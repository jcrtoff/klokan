...
const { createSession, getSession, saveMessage, updateLeadProfile, updateStats, assignBroker, setControlledBy, archiveSession, getFilteredSessionList, enrichSessionList } = require('./sessions');
    ...
    ws.on('message', async (raw) => {
      ...
      switch (msg.type) {
        ...
        case 'archive_session': {
          if (!ws.brokerId) break;
          const session = await getSession(msg.sessionId);
          if (!session) break;

          const canArchive =
            ws.brokerRole === 'admin' ||
            session.assignedBrokerId === ws.brokerId ||
            (ws.brokerRole === 'manager' && session.assignedBrokerManagerId === ws.brokerId);
          if (!canArchive) break;

          broadcast(session.chatClients, { type: 'session_closed', sessionId: session.id });
          await archiveSession(session.id);

          const archivedMsg = JSON.stringify({ type: 'session_archived', sessionId: session.id });
          for (const client of brokerClients) {
            if (client.readyState === 1) client.send(archivedMsg);
          }
